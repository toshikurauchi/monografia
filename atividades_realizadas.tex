...
Na \texttt{Pandora's Box}, \texttt{buffer}s são implementações da interface \texttt{Buffer}. 

% falta descrever os detalhes de implementação
Exemplo de uso de um \texttt{buffer} da \texttt{Pandora's Box}:
\begin{verbatim}
GraphicAPI *gfx = ...;
Buffer *buffer = gfx->getFactory()->createBuffer(size);
...
Com o aumento da complexidade dos modelos, foi necessário reinventar a API existente, com isso foi criado o conceito de \texttt{vertex array}, que é simplesmente um vetor de valores de tipos variados em que cada atributo é especificado através de seu tipo primitivo (double, float, int, short, byte) número de componentes, significado (posição, vetor normal, coordenada de textura, \ldots) e a distância em bytes para o próximo valor desse atributo no vetor. Essa estrutura é exemplificada na figura abaixo.

\includegraphics{vertexbuffer}
...
% TODO: explicar os built-ins

O mecanismo para o envio dos atributos é semelhante ao utilizada para enviar as uniformes \texttt{built-in}. Cada um dos tipos de atributo definidos na \texttt{enum} \texttt{pbge::VertexAttrib::Type} tem uma classe especializada em associar o atributo ao shader.
