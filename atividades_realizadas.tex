...
especificar o código fonte que será utilizado por meio da função
...
para o objeto vazio criado, em seguida é necessário associar o objeto criado ao contexto 
...
Após a criação do handler, a textura deve ser associada ao contexto através da
chamada a \texttt{glBindTexture(GL\_TEXTURE\_BUFFER, handler)}. O buffer a
ser utilizado pela textura é então definido pelo comando \texttt{glTexBuffer}.
...
% Aparentemente as duas últimas funções são a mesma. Está certo isso?
...
abstrata \texttt{FramebufferObject}. A criação e uso de framebuffer objects
