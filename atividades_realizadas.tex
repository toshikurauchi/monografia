...
A Pandora's Box utiliza uma árvore (um grafo direcionado, sem circuitos e com uma raíz), conhecida como grafo de cena, para representar a estrutura de uma cena. As mudanças realizadas por cada nó são aplicadas somente a si mesmo e aos seus filhos.
...
\begin{itemize}
\item \textbf{TransformationNode}: Esse nó guarda uma matriz de transformação $T$. No \texttt{updatePass} e \texttt{renderPass} a matriz corrente $M$ é armazenada e multiplicada por $T$ e o resultado é utilizado como a nova matriz de transformação corrente. Então no \texttt{postUpdatePass} e \texttt{postRenderPass} $M$ é reatribuida à matriz corrente.
% Explicar os outros tipos de nó
\end{itemize}
...
O \texttt{FramebufferImageProcessor} é utilizado para fazer o pós-processamento da imagem que estiver no buffer chamado color dentro do \texttt{FramebufferObject} atual. O algorirtmo executado por esse objeto é descrito pelo algoritmo abaixo:
...
\begin{center}
\begin{longtable}{cc}
\epsfig{file=brain1.eps, width=0.5\linewidth,clip=} &
\epsfig{file=brain2.eps, width=0.5\linewidth,clip=} \\
\epsfig{file=brain3.eps, width=0.5\linewidth,clip=} &
\epsfig{file=brain4.eps, width=0.5\linewidth,clip=} \\
\epsfig{file=brain5.eps, width=0.5\linewidth,clip=} &
\epsfig{file=brain6.eps, width=0.5\linewidth,clip=} \\
\end{longtable}
\end{center}

As imagens acima foram extraídas da aplicação de visualização de campos tensoriais. Cada imagem mostra o campo tensorial de um cérebro humano sendo descartados alguns tensores de acordo com o valor de anisotropia fracionada.
