...
Mas esse modelo não solucionava o problema de tráfego de dados entre a memória principal (RAM) e a memória de vídeo (VRAM). A solução do problema de tráfego de dados foi colocar o \textt{vertex array} dentro de um buffer gerenciado pela implementação da API gráfica, assim, a implementação poderia colocar dados muito utilizados dentro de regiões de fácil acesso, como a VRAM. Essa solução ficou conhecida como \texttt{Vertex Buffer}
...
% Eu sei que tá zuado mas é só pra pensar um poko...
Uniforms são parâmetros constantes por primitiva. São passados para o shader através da classe UniformSet.

Atributos são parâmetros constantes por vértice da primitiva. São passados para o shader através de vertex buffers.
