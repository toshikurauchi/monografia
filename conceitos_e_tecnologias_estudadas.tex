...
Que recebe como parâmetros um ponteiro para uma estrutura do tipo \texttt{MSG}\cite{docmsg}, o handler retornado pela função \texttt{CreateWindow}, wMsgFilterMin e wMsgFilterMax são constantes que podem ser passadas para indicar que tipo de mensagem deve ser retornada. O valor de retorno dessa função é um inteiro que indica qual o tipo de mensagem foi retornada ou se ocorreu um erro durante a execução da função.

Após recuperar a mensagem ela precisa ser traduzida e em seguida enviada para o seu destino final, a função que cuida da tradução da mensagem é\cite{doctranslatemessage}
...
Quando a thread principal recebe a mensagem \texttt{WM\_QUIT}, o retorno de \texttt{GetMessage} é 0, nesse momento a tradução e envio de mensagens deve parar e o programa deve apagar o registro da classe criada por \texttt{RegisterClass}. O registro é apagado pela função\cite{docunregisterclass}:
