...
\item \texttt{hWndParent}: se a janela que está sendo criada é uma subjanela de uma já existente, passa-se o handle da janela pai nesse argumento, caso contrário, passa-se \texttt{NULL}. Para a engine gráfica, esse argumento sempre recebe o valor \texttt{NULL}.
\item \texttt{hMenu}: se for necessário criar uma menu, deve-se passar o handle do menu nesse argumento, caso contrário, passa-se \texttt{NULL}. %Falar o que é um handler
...
Onde \texttt{hWnd} é o handle devolvido pela função \texttt{CreateWindow} e \texttt{nCmdShow} controla como a janela deve ser exibida (maximizada, minimizada, simplesmente exibida, \ldots)
