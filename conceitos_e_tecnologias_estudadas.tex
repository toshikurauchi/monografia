...
% Tirei o qualificador.. o que é isso?
\begin{verbatim}
...
O OpenGL é uma especificação aberta e extensível de uma interface de software para o hardware gráfico.

%todo: falar sobre a extensibilidade é importante!!!
...
No primeiro são aplicadas as transformações lineares e necessárias nos vértices (vertex shader). Em seguida as primitivas são subdivididas em partículas menores e podem ser gerados novos vértices (tessellation shaders e geometry shader). Por fim os vértices em 3 dimensões são projetados em um plano (rasterização) que representa a tela resultando em pixels que por sua vez passam por um processamento final (fragment shader). Esse processo é conhecido como pipeline.

\includegraphics{pipeline}

No OpenGL todos esses processos (com exceção da rasterização) são customizáveis, entretanto na Pandora's Box somente o vertex shader e o fragment shader podem ser modificados. Mais detalhes são apresentados na seção seguinte.

\subsection{GLEW}

A \texttt{OpenGL Extension Wrangler Library}  \cite{glew}, conhecida como \texttt{GLEW} é uma biblioteca que auxilia no gerenciamento de extensões do OpenGL. Ela é capaz de obter as funções definidas por extensões do OpenGL e além disso, fornece constantes definidas em tempo de execução que contém as extensões do OpenGL que o sistema do usuário suporta. %Muita repetição da palavra extensões
...
Onde \texttt{hWnd} é o handler devolvido pela função \texttt{CreateWindow} e \texttt{nCmdShow} controla como a janela deve ser exibida (maximizada, minimizada, simplesmente exibida, \ldots)
...
A criação do contexto é gerenciada pela API do wgl (também conhecido como Wiggle), que é a implementação de OpenGL para o Windows. O contexto que será utilizado pela aplicação deve ser criado através da função wglCreateContext \cite{wglcreatecontextdoc}: %TODO essa citação está como [?]

\begin{verbatim}
% assinatura da wglCreateContext
\end{verbatim}

Essa função tem como argumento o handler para o dispositivo utilizado pela janela. Tal handler pode ser obtido através da função GetDC \cite{getdcdoc}: %TODO essa citação está como [?]
...
essa função retorna o handler para a biblioteca carregada ou NULL caso tenha ocorrido um erro. Caso o carregamento seja bem sucedido, pode-se utilizar o handler obtido para procurar ponteiro de funções.
