...
programação estruturada quanto orientada a objetos.Ela foi desenvolvida como uma
evolução da linguagem C e, por esse motivo, suas sintaxes são semelhantes.
...
O módulo \texttt{function} cria functors (instancias de classes que implementam
...
Os módulos bind e function da boost são utilizados principalmente em conjunto
com algoritmos da STL, como o std::for\_each:
...
\texttt{GLEW} é uma biblioteca que auxilia no gerenciamento de extensões do OpenGL. 
...
programa será executado. Nesse trabalho foi utilizada apenas a API do sistema operacional Windows.
