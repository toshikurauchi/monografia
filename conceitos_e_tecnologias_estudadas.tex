...
Além disso ele não disponibiliza nenhuma forma de leitura de arquivos. Por esse
motivo, não é possível visualizar diretamente imagens guardadas em arquivos
(como png, jpg, bmp, ou modelos em três dimensões como 3ds~\cite{3dsfileformat},
blend~\cite{blendfileformat}, ou obj~\cite{objfileformat}).
...
de anisotropia de um processo de difusão. Sejam $\lambda_{1}, \lambda_{2}, \lambda_{3}$ os autovalores do tensor. A anisotropia fracionada ($AF$) pode ser calculada a partir da seguinte fórmula~\cite{hagmann}:
...
planar e esférica) que indicam a forma da difusão~\cite{westinISMRM97}. Sejam $\lambda_{1} \geq \lambda_{2} \geq \lambda_{3} \geq 0$:
