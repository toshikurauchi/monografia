...
Ou seja, para declarar uma variável inteira chamada \texttt{contador} com valor inicial zero será utilizada a seguinte sintaxe:
...
As primitivas geradas na CPU (Central Processing Unit) são enviadas para a GPU (Graphics Processing Unit) onde serão processadas e utilizadas para gerar uma imagem que será mostrada na janela. %TODO Explicar cada parte, falar o que são shaders e depois falar o nome Pipeline 

\includegraphics{pipeline}

\subsection{GLEW}

A \texttt{OpenGL Extension Wrangler Library}  \cite{glew}, conhecida como \texttt{GLEW} é uma biblioteca que auxilia no gerenciamento de extensões do OpenGL. Ela é capaz de obter as funções definidas por extensões do OpenGL e além disso, fornece constantes definidas em tempo de execução que contém as extensões do OpenGL que o sistema do usuário suporta.
...
glewInit();
// após a inicialização as funções definidas através de extensões estão disponíveis
